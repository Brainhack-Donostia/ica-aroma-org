...
def main(args):
    ...
    if args.inFeat:
        ...
        inFile = op.join(args.inFeat, "filtered_func_data.nii.gz")
        mc = op.join(args.inFeat, "mc", "prefiltered_func_data_mcf.par")
        affmat = op.join(args.inFeat, "reg", "example_func2highres.mat")
        warp = op.join(args.inFeat, "reg", "highres2standard_warp.nii.gz")
        ...
        if op.isdir(op.join(args.inFeat, "filtered_func_data.ica")):
            melDir = op.join(args.inFeat, "filtered_func_data.ica")
        else:
            melDir = args.melDir
    else:
        inFile = args.inFile
    ...
    # Parse the arguments which do not depend on whether a Feat directory has
    # been specified
    outDir = args.outDir
    dim = args.dim
    ...
    if (
        not (denType == "nonaggr")
        and not (denType == "aggr")
        and not (denType == "both")
        and not (denType == "no")
    ):
        print(
    ...
    if op.isdir(outDir) and args.overwrite is False:
        print(
            "Output directory",
            outDir,
            """already exists.
        ...
        exit()
    elif op.isdir(outDir) and args.overwrite is True:
    ...
    # creating a new one.
    mask = op.join(outDir, "mask.nii.gz")
    if args.mask:
        ...
        if args.inFeat and op.isfile(op.join(inFeat, "example_func.nii.gz")):
            os.system(
                " ".join(
                    [
                        op.join(fslDir, "bet"),
            ...
            os.rename(op.join(outDir, "bet_mask.nii.gz"), mask)
    ...
    edgeFract, csfFract = aromafunc.feature_spatial(
    ...
    motionICs = aromafunc.classification(
        outDir,
        maxRPcorr,
        edgeFract,
        HFC,
    ...
    if args.generate_plots:
        ...
        classification_plot(op.join(outDir, "classification_overview.txt"),
                            outDir)
